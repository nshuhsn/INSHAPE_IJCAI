...
def reinmax(
    ...
    if not training:
        shape = logits.size()
        logits = logits.view(-1, shape[-1]) # B*M*L, 2

        y_soft = logits.softmax(dim=-1) # B*M*L, 2
        y_hard = (y_soft > 0.5).float() 

        m = (y_hard - y_soft).detach() + y_soft
        return m.reshape(shape), y_soft.view(shape)[...,1:2]
    ...
    return grad_sample.view(shape), y_soft.view(shape)[...,1:2]